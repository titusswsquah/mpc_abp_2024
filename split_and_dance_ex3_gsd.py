...
from lib.utils import array_module, asnumpy
...
class SetPoint:
    def __init__(self):
        ...
        self.center1 = -2.5
        self.center2 = 2.5
        self.trigger_times = [0, 10, 15, 25, 35, 65, 75, 85, 115, 125]
        self.sim_time = 140
        self.w001_max = 3

        self.amp = 3

        self.int1 = interp1d([self.trigger_times[1], self.trigger_times[2]],
                             [self.center, self.center1],
                             fill_value=(self.center, self.center1),
                             bounds_error=False)
        self.int2 = interp1d([self.trigger_times[1], self.trigger_times[2]],
                             [self.center, self.center2],
                             fill_value=(self.center, self.center2),
                             bounds_error=False)

        self.int3 = interp1d([self.trigger_times[3],
                              self.trigger_times[4],
                              self.trigger_times[5],
                              self.trigger_times[6]],
                             [1, self.amp, self.amp, 1], fill_value=(1, 1),
                             bounds_error=False)
        self.int4 = interp1d([self.trigger_times[6],
                              self.trigger_times[7],
                              self.trigger_times[8],
                              self.trigger_times[9]],
                             [1, self.amp, self.amp, 1],
                             fill_value=(1, 1), bounds_error=False)
...
def main():
    ...
    walls = [
        hoomd.wall.Plane((0, -plate_gap / 2 - 2 ** (1 / 6) * a, 0),
                         (0, 1, 0)),
        hoomd.wall.Plane((0, plate_gap / 2 + 2 ** (1 / 6) * a, 0),
                         (0, -1, 0))]
...
class ControlPolicy(BaseControlPolicy):
    def __init__(self,
        ...
        pcl_pos = np.clip(frame.particles.position[:, 1], -self.plate_gap / 2,
                          self.plate_gap / 2)
        pcl_quarts = xp.asarray(frame.particles.orientation)
        pcl_thetas = \
        hoomd_utils.quaternion_to_euler_angle_vectorized(pcl_quarts[:, 0],
                                                         pcl_quarts[:, 1],
                                                         pcl_quarts[:, 2],
                                                         pcl_quarts[:, 3])[
            -1] * np.pi / 180
        self.bd_zpts = np.linspace(-box_height / 2, box_height / 2, 1001)
        self.meas_zpts = np.linspace(-self.plate_gap / 2, self.plate_gap / 2,
                                     301)
        ...
        self.constrained_zpts = np.linspace(-self.plate_gap / 2,
                                            self.plate_gap / 2, 80)
        ...
        state_pts_all, self.state_dz, self.state_dz2, self.state_ig = utils.colloc(
            self.n_state_pts,
            left=True,
            right=True,
        ...
        self.wr001_pts, self.wr001_dz, self.wr001_dz2, self.wr001_ig = utils.colloc(
            self.n_wr001_pts,
            plate_gap=self.plate_gap,
            shifted=False)
        ...
        self.state_a_mat, self.state_ctrl_mats, self.inv22a21 = pars_to_state_space_v2(
            state_pars_dict,
            return_inv_mat=True)
        ...
        half_pts, _, _, self.diff_ig = utils.colloc(31,
                                                    plate_gap=self.plate_gap / 2,
                                                    shifted=True)
    ...
    def con(self, x, u):
        bound = (self.wr001_to_constrained_z @ u) ** 2 - self.w001_max ** 2
        return bound
    ...
    def stagecost(self, x, u, x_sp, u_sp, Deltau):
        ...
        state_penalty = self.state_cost_multiplier * mpc.util.mtimes(
            self.state_ig[1:-1].T, integrand)
        diff_penalty = self.diff_cost_multiplier * (
                    amp1 * mpc.util.mtimes(self.diff_ig.T, integrand1)
                    - amp2 * mpc.util.mtimes(self.diff_ig.T, integrand2)) ** 2

        d2udz2 = mpc.util.mtimes(self.wr001_dz2, u)
        dz2_ctrl_penalty = self.dz2_ctrl_cost_multiplier * mpc.util.mtimes(
            d2udz2.T, d2udz2)
        dt_penalty = self.dt_ctrl_cost_multiplier * mpc.util.mtimes(Deltau.T,
                                                                    Deltau)
        penalty = state_penalty + diff_penalty + dz2_ctrl_penalty + dt_penalty
        return penalty
    ...
    def get_state_cost(self, x, x_sp):
        ...
        diff_penalty = self.diff_cost_multiplier * (
                    amp1 * mpc.util.mtimes(self.diff_ig.T, integrand1)
                    - amp2 * mpc.util.mtimes(self.diff_ig.T, integrand2)) ** 2
        return state_penalty + diff_penalty
    ...
    def get_ctrl_cost(self, u, Deltau):
        ...
        dt_penalty = self.dt_ctrl_cost_multiplier * mpc.util.mtimes(Deltau.T,
                                                                    Deltau)
        return dz2_ctrl_penalty + dt_penalty
    ...
    def termcost(self, x, x_sp):
        ...
        diff_penalty = self.diff_cost_multiplier * (
                    amp1 * mpc.util.mtimes(self.diff_ig.T, integrand1)
                    - amp2 * mpc.util.mtimes(self.diff_ig.T, integrand2)) ** 2
        penalty = state_penalty + diff_penalty
        return penalty
    ...
    def set_forces(self, timestep):
        ...
        if timestep % int(self.ctrl_dt / self.sim_dt) == 0:
            ...
            if 'Infeasible_Problem_Detected' not in self.controller.stats[
                "status"]:
                root_vals = np.squeeze(self.controller.var["u", 0]).reshape(-1,
                                                                            1)
