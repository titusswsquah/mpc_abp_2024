...
def main(make_pdf_, show_):
    ...
    sp_color_ind = 4
    mpc_color_palette = [color_brewer.rgb_to_hex(e)
                         for e in color_brewer.Blues[3]]
    ...
    ax[2].set_ylabel(r'$x$-torque ${\omega}_{x}$')
    ...
    def animate(i):
        ...
        udata1 = asnumpy(xp.polyval(
            xp.array(frame.log['wr001_coeffs']),
            xp.array(xdata1)
        ))
        lines = []
        ...
        bd_vl.set_xdata([sp_fcn(time),
                         sp_fcn(time)])
        lines.append(bd_vl)
        field_vl.set_xdata([sp_fcn(time),
                            sp_fcn(time)])
